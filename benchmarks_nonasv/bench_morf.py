...
```bash
python bench_morf.py bench ~/bench_results_forest pr
```

2. On main run:

```bash
python bench_morf.py bench ~/bench_results_forest main
```

3. Plotting

```bash
python bench_morf.py plot ~/bench_results_forest
    ...
    (
        ...
        list(
            product(
                ["gini", "entropy"],
                [
                    partial(make_classification_custom, n_informative=100, n_classes=5),
                    make_low_card_data,
                ],
                [10_000],
                ["dense"],
    ...
    with results_path.open("w") as f:
        ...
        for Klass, items in benchmark_config:
            for config in items:
                ...
                print(f"{combine_config} with {results_mean:.3f} +/- {results_stdev:.3f}")
