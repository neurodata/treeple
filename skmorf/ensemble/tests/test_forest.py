...
def check_class_weights(name):
    ...
    clf3 = ForestClassifier(
        class_weight=[{0: 2.0, 1: 2.0, 2: 1.0}, {0: 2.0, 1: 1.0, 2: 2.0}, {0: 1.0, 1: 2.0, 2: 2.0}],
        random_state=0,
    )
...
@pytest.mark.parametrize(
    ...
    [
        (int(1e9), ValueError, "`max_samples` must be in range 1 to 6 but got value 1000000000"),
        ...
        (np.nan, ValueError, r"`max_samples` must be in range \(0, 1\) but got value nan"),
        (np.inf, ValueError, r"`max_samples` must be in range \(0, 1\) but got value inf"),
        (
            "str max_samples?!",
            TypeError,
        ...
        (
            np.ones(2),
            TypeError,
            r"`max_samples` should be int or float, but got type " r"'\<class 'numpy.ndarray'\>'",
        ),
    ],
)
...
def test_criterion_deprecated(old_criterion, new_criterion):
    ...
    with pytest.warns(FutureWarning, match=f"Criterion '{old_criterion}' was deprecated"):
        est1.fit(X, y)
