...
- |Enhancement| :func:`sktree.datasets.make_trunk_classification` now
  has a generative model based on Trunk and banded covariance, 
  :func:`sktree.datasets.approximate_clf_mutual_information` and
  :func:`sktree.datasets.approximate_clf_mutual_information_with_monte_carlo` to
  approximate mutual information either numerically or via Monte-Carlo
  by `Adam Li`_ and `Haoyin Xu`_ (:pr:`#199`).
