...
def apply_marginal(est, X, S, weighted: bool = False, check_input=True):
    ...
    """
    check_is_fitted(est)
    X = est._validate_X_predict(X, check_input)
    ...
    X_leaves = _apply_marginal_func(est, X, S, weighted=weighted)
    return X_leaves


def _apply_marginal_forest(est, X, S, weighted: bool = False):
    ...
    results = Parallel(
        ...
        verbose=est.verbose,
        prefer="threads",
    )(delayed(_apply_marginal_tree)(tree, X, S, weighted) for tree in est.estimators_)
    return np.array(results).T


def _apply_marginal_tree(est: BaseDecisionTree, X, S, weighted: bool = False):
    ...
    X_leaves = apply_marginal_tree(est.tree_, X, S, weighted, random_state=est.random_state)
    return X_leaves
